...
class conv_base_seq extends uvm_sequence #(conv_drv_item);
    ...
    bit signed [WGT_WIDTH-1:0] kernel [K_DIM*K_DIM];   // K_DIM*K_DIM for K_DIM=3
    bit [PIX_WIDTH-1:0]        image  [IMG_MAX_H][IMG_MAX_W]; // IMG_MAX dimensions
        ...
        for (int r = 0; r < IMG_MAX_H; r++) begin
            for (int c = 0; c < IMG_MAX_W; c++) begin
                ...
                if (idle_max == 0) 
                    ...
                    item.idle_cycles = item.idle_cycles % (idle_max+1);
    ...
    task wait_done();
        conv_drv_item t = conv_drv_item::type_id::create("t_wd");
        start_item(t);
        t.kind = TX_WAIT_DONE;
        finish_item(t);
    endtask
